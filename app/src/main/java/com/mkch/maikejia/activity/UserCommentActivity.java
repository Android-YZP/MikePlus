...
import com.mkch.maikejia.fragment.UserCommentMineListFragment;
import com.mkch.maikejia.fragment.UserCommentReceiveListFragment;
...
public class UserCommentActivity extends FragmentActivity {
	...
	private UserAttentionTabBarLayout mUserCommentTabBarLayout;
	private ViewPager mViewPagerUserComment;
	...
	private void initView() {
		...
		mUserCommentTabBarLayout = (UserAttentionTabBarLayout)findViewById(R.id.custom_user_attention_tabbar);
		mViewPagerUserComment = (ViewPager)findViewById(R.id.viewPager_user_attention);
		//初始化屏幕宽度
	...
	private void initData() {
		mTvTitle.setText("我的评论");
		mUserCommentTabBarLayout.setData("我的评论", "收到的评论");
		mViewPagerUserComment.setAdapter(new UserAttentionFragmentPagerAdapter(getSupportFragmentManager()));
	}
	...
	private void setListener() {
		...
		mUserCommentTabBarLayout.setOnItemClickListener(new IUserAttentionTabBarCallBackListener() {
			@Override
			public void clickItem(int id) {
				switch (id) {
				case R.id.user_attention_item0:
					mViewPagerUserComment.setCurrentItem(FLAG_ITEM_0);//点击后设置当前页是显示页
					break;
				case R.id.user_attention_item1:
					mViewPagerUserComment.setCurrentItem(FLAG_ITEM_1);
					break;
				}
			}
		});
		
		mViewPagerUserComment.setOnPageChangeListener(new OnPageChangeListener() {
			
			@Override
			public void onPageSelected(int position) {
				mUserCommentTabBarLayout.changeTabBarItems(position);
	...
	private class UserAttentionFragmentPagerAdapter extends FragmentPagerAdapter{
		...
		public Fragment getItem(int postion) {
			switch (postion) {
			case FLAG_ITEM_0:
				return new UserCommentMineListFragment();
			case FLAG_ITEM_1:
				return new UserCommentReceiveListFragment();
			}
			return null;
		}
